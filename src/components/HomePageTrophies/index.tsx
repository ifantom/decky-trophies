...
import { getMyTrophies } from "../../functions";
...
export default function HomePageTrophies({ settings }: { settings: SettingsBus }): ReactElement {
  ...
  useEffect(() => {
    return settings.subscribe((value) => {
      if (value.homePageTrophiesShow !== show) setShow(value.homePageTrophiesShow);
      if (value.homePageTrophiesPosition !== position) setPosition(value.homePageTrophiesPosition);
